...
void Generator::load(std::istream& in) {
    // format: name power fuelType fuelConsumption
    in >> name >> power >> fuelType >> fuelConsumption;
    if (!in) {
        throw std::runtime_error("Pomylka vvedennia dlia Generator");
    }
    if (power < 0 || fuelConsumption < 0) {
        throw std::runtime_error("Vidjemni znachennia u Generator");
    }
}
...
void Generator::loadFromFile(std::istream& in) {
    in >> name >> power >> fuelType >> fuelConsumption;
    if (!in) {
        throw std::runtime_error("Nevdalyi chytannia Generator z failu");
    }
    if (power < 0 || fuelConsumption < 0) {
        throw std::runtime_error("Vidjemni znachennia u Generator (fail)");
    }
}
